...
# # Mostrar columnas y primeras filas para depurar (puedes comentar luego)
# st.write("Columnas del DataFrame:", df.columns.tolist())
# st.write(df.head())

# En tu dataset, la columna que indica el año es 'model_year', no 'year'
if 'model_year' not in df.columns:
    st.error("La columna 'model_year' no se encontró en el archivo CSV. Por favor verifica el nombre de la columna.")
    st.stop()
...
df_filtered = df[(df['model_year'] >= year_filter[0]) & (
    df['model_year'] <= year_filter[1]) & (df['fuel'].isin(selected_fuels))]
...
if show_scatter_year:
    ...
    fig_year = px.scatter(
        df_filtered, x="model_year", y="price",
        color="fuel",
        labels={"model_year": "Año de Fabricación", "price": "Precio (USD)"},
        title="Precio vs Año de Fabricación por Tipo de Combustible",
        color_discrete_sequence=px.colors.qualitative.Vivid,
        hover_data=['odometer', 'model', 'condition']
    )
    st.plotly_chart(fig_year, use_container_width=True)

# Estadísticas descriptivas básicas
st.subheader("Estadísticas Descriptivas")
st.write(df_filtered[['price', 'odometer', 'model_year']].describe())
